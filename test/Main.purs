...
main = do
  ...
  ab1 <- execPut $ Test.putMsg1 msg1
  let dv1 = DV.whole ab1
  parseResult1 <- runParserT dv1 $ Test.parseMsg1 $ DV.byteLength dv1
  ...
  let msg2 = (Test.Msg2
    ...
    , f13: replicate 3 "🍝"
    -- -- , f14 :: Maybe.Maybe ArrayBuffer.Types.ArrayBuffer
    , f15: replicate 3 Test.Msg2_E1_Three
    })

  ab2 <- execPut $ Test.putMsg2 msg2
  let dv2 = DV.whole ab2
  parseResult2 <- runParserT dv2 $ Test.parseMsg2 $ DV.byteLength dv2
  ...
  let msg3 = (Test.Msg3
    { f1: replicate 3 msg1
    , f2: replicate 3 msg2
    -- , f4: replicate 3 (Test.Msg3_Msg4 { f1: Just 1 })
    })

  ab3 <- execPut $ Test.putMsg3 msg3
  let dv3 = DV.whole ab3
  parseResult3 <- runParserT dv3 $ Test.parseMsg3 $ DV.byteLength dv3
  case parseResult3 of
    Left err -> assert' ("msg3 parse " <> show err) false
    Right (Test.Msg3 {f1:f1',f2:f2'}) -> assert' "msg3 roundtrip" $
      (and $ zipWith genericEq f1 f1') && (and $ zipWith genericEq f2 f2')
     where
      Test.Msg3 {f1,f2} = msg3
