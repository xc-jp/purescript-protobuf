...
genFile (FileDescriptorProto
 ...
 where
  ...
  fileNameOut = capitalize baseName <> ".purs"
  ...
  content = String.joinWith "\n" $
    ...
    , "module " <> (String.joinWith "." ((map capitalize packageName))) <> "blahbal"
    , "( " <> (String.joinWith "\n, "
...
-- | We also get the cycle error if we try to give the messages Show instances.
  ...
  genFieldPut nameSpace (FieldDescriptorProto
   ...
   where
    ...
    f LABEL_REPEATED TYPE_MESSAGE (Just tname) =
      "  Runtime.putRepeated " <> show fnumber <> " r." <> fname <> " $ Runtime.putLenDel put" <> mkFieldType nameSpace tname
    f _ TYPE_MESSAGE (Just tname) =
      "  Runtime.putOptional " <> show fnumber <> " r." <> fname <> " $ Runtime.putLenDel put" <> mkFieldType nameSpace tname
  ...
  genFieldParser nameSpace (FieldDescriptorProto
   ...
   where
    ...
    f LABEL_REPEATED TYPE_MESSAGE (Just tname) = String.joinWith "\n"
      [ "  parseField " <> show fnumber <> " Common.LenDel = do"
      , "    x <- Runtime.parseLenDel parse" <> mkFieldType nameSpace tname
      , "    pure $ Record.Builder.modify (Symbol.SProxy :: Symbol.SProxy \"" <> fname <> "\") $ Function.flip Array.snoc x"
      ]
  ...
  genFieldRecord nameSpace (FieldDescriptorProto
   ...
   where
    ...
    ptype LABEL_REPEATED TYPE_MESSAGE (Just tname) = "Array " <> mkFieldType nameSpace tname
    ptype LABEL_REPEATED TYPE_BYTES _ = "Array ArrayBuffer.Types.ArrayBuffer"
    ptype LABEL_REPEATED TYPE_UINT32 _ = "Array UInt.UInt"
    ptype LABEL_REPEATED TYPE_ENUM (Just tname) = "Array " <> mkFieldType nameSpace tname
    ...
    ptype _ TYPE_MESSAGE (Just tname) = "Maybe.Maybe " <> mkFieldType nameSpace tname
    ptype _ TYPE_BYTES _ = "Maybe.Maybe ArrayBuffer.Types.ArrayBuffer"
    ptype _ TYPE_UINT32 _ = "Maybe.Maybe UInt.UInt"
    ptype _ TYPE_ENUM (Just tname) = "Maybe.Maybe " <> mkFieldType nameSpace tname
  ...
  mkFieldType
    ...
    -> String -- package-qualified field name
    -> String
  mkFieldType nameSpace s =
    let (ScopedField packageNames name) = parseFieldName s
    in
    if packageNames == []
      then mkTypeName $ nameSpace <> [name]
      else String.joinWith "." $ map capitalize $ packageNames <> [name]
